...
| Phase 4.42 | ✅ Done | **七大提前佈局策略**：`InstAccumDetector`（法人建倉：距60D高點-15~-40%，法人連買≥3天）✅ · `ChipTransferDetector`（籌碼轉移：K-of-N 5訊號，融資連降+法人買+大戶增+散戶退）✅ · `VCPDetector`（Minervini VCP波動收縮：連續回調幅度遞減+量能乾涸）✅ · `HTFDetector`（旗形：先漲25%+再低量整理<15%）✅ · `_scan_early_accum_batch` 整合進 `make plan`（同批 tickers 四引擎並跑）✅ · 產業輪動加分（ROTATION_EMERGING +5 / HOT +3 / COOLING -3，讀 rotation_signal.json）✅ · 新信號類型：法人建倉/籌碼轉移/VCP/旗形（HTML + 終端顯示）✅ · 30 新單元測試 ✅ · 653 unit tests passing ✅ |
| Phase 4.43 | ✅ Done | **連續評分（消除分數撞群）**：`_ScoreBreakdown` 全 63 個 `_pts` 欄位 int→float ✅ · `SignalOutput.confidence` int→float（移除 le=100 上限）✅ · 8 個高方差因子改連續函數：`_volume_ratio_score`（log 曲線 1x→0/2x→5.05/3x→8.0 peak/4x+衰減）、`_close_strength_score`（(ratio-0.5)*8 線性，clamp[-2,4]）、`_rsi_momentum_score`（三角形 50-60→4，40/70→0）、`_volume_dryup_score`（線性 1.0→0/0.4→8）、`_proximity_score`（帳篷型 0.85→0/0.95→12 peak/0.99→0）、`_bb_compression_score`（線性 0.04→10/0.16→0）、`cumul_flow_pts`（log 曲線）、`_institution_strength_pts`（線性插值）✅ · `proximity_pts == 12` 觸發點改 `>= 11.5`（float-tolerant）✅ · `signal_recorder` 寫 DB 前 `int(round())` 保存（DB 仍 int）✅ · HTML 顯示 1 位小數（`conf_disp`），`data-conf` 維持 int 給 slider ✅ · 終端 `_conf_bar` + `analyze.py` 標題改 1 位小數 ✅ · 12 個整數斷言改 `pytest.approx` ✅ · 670 unit tests passing ✅ |
| Phase 4.44 | ✅ Done | **TCE 完成連續評分**：剩餘 ~50 個小桶因子全改連續函數 ✅ · **Pillar 1**：`_vwap_advantage_score`（線性 0%→0 / 6%→6）、`_trend_continuity_score`（線性 0-4 up bars / consec≥3 floor）、`_volume_escalation_score`（base 3.0 + 0-2.0 bonus）、`_volume_climax_score`（spike × dryup 乘積）、`_ma5_walk_score`（線性 0.5→0 / 1.0→2）、`_dmi_initiation_score`（ADX 20→0.6x / 25+→1.0x 縮放，保留 cross/init flag）✅ · **Pillar 2A**：`breadth_pts`（線性 1→1 / 20→10）、`concentration_pts`（線性 0.20→0 / 0.45→10，thin-market cap 5）、`_compute_continuity_pts`（連續 overlap × 1.67 + 3day avg bonus）、`foreign_broker_pts`（線性 0.25→3 / 0.45→5）✅ · **Pillar 2B**：所有因子改連續（`institution_continuity` days→pts 線性、`margin_utilization` 雙向線性 0.30→0 中心、`sbl_pressure` 0.05→0 / 0.20→-8、`consistent_accum` ratio→pts、`inst_synergy` pct 分段線性、`ownership_concentration` magnitude based、`obv_accumulation` slope based 含 PRIME bonus、`vol_asymmetry` ratio based、`dual_inst_flow` min(F,T) ratio、`chip_cleanliness` K/6×10、`turnover` regime-aware、`super_large` pct + count bonus、`foreign_trend`/`short_cover`/`large_2w_trend`/`inst_accel_3d` 全線性、`margin_persist_decline` streak 線性、`chip_concentration_accel` magnitude based、`short_squeeze_setup` SMR 線性、`stealth_accum_composite` count×factor）✅ · **Pillar 3**：`ma_convergence`（線性 spread）、`consolidation_weeks`（線性 weeks）、`inside_bar_streak`（已連續）、`prior_advance`（線性 advance%）、`ma_alignment`（partial credit 2.5/pair）、`ma20_slope`（線性 magnitude）、`relative_strength`/`longterm_rs`（線性 outperform）、`near_highhist`（線性 ratio）、`bb_upper_walk`（線性 near_upper count）✅ · **Risk deductions**：`long_upper_shadow`/`recent_advance_deduction`/`adx_exhaustion`/`daytrade_heat`/`sbl_breakout_fail`/`margin_chase_heat`/`vol_consecutive_surge` 改連續，gate-style 保持門檻 ✅ · 6 個整數斷言改 `pytest.approx` ✅ · 670 unit tests passing ✅ |
| Phase 4.45 | ✅ Done | **SurgeRadar 連續評分**：`raw`/`breakdown` int→float ✅ · `score` 不再 `min(100,..)` clamp 為 int，輸出 float（顯示 1 位小數）✅ · 24 個 `_score_*` 方法簽名 `int`→`float` ✅ · **8 高方差因子**改連續：`vol_ratio`（log 曲線 1.5→6/3.0→10 peak/5.0+ 衰減）、`close_strength`（線性 (ratio-0.3)*11.4 clamp[2,8]）、`inst_buy_fresh`（base × net_buy intensity）、`inst_cumulative_flow`（log 曲線含 accel/quiet bonus 連續）、`inst_synergy`（pct 分段線性）、`ownership_concentration`（magnitude based）、`breakout_20d`（突破幅度線性 0%→6/5%→10）、`rsi_healthy`（55-70 peak 5/70+ 平穩 3 直到 95+ 才略降）✅ · **10 中等因子**改連續：`industry_strength`（線性 60→5/80→10）、`pocket_pivot`（vol/max_down 線性）、`breakaway_gap`（gap_pct 線性）、`relative_strength`（diff 線性 0.5%→4/2%→8）、`bb_squeeze_breakout`（expansion ratio 線性）、`margin_not_hot`（雙向線性 0→4/0.25→0）、`ma5_walk`（ratio 線性 0.5→0/1.0→2）、`bb_upper_walk`（day3→-2/day5→-3）、`market_heat`（industry rank 線性 + 概念計數加成）✅ · **6 低影響因子**：`foreign_trend`/`short_cover`/`large_2w_trend`/`inst_accel_short` 全線性（gate-style daytrade_penalty/taifex_context/margin_declining 保留 binary）✅ · `surge_scan.py` 顯示 1 位小數 ✅ · `_print_score_health` 改 float 簽名 ✅ · DB schema `score FLOAT` 已支援不需 migration ✅ · 18 個整數斷言改 `pytest.approx`/區間 ✅ · 671 unit tests passing ✅ |
| Phase 4.46 | ✅ Done | **早期佈局偵測器連續評分**：5 個 detector 全改連續，所有 score 為 float ✅ · **InstAccumDetector**：consec_days 線性（3→10/5→18/8+→25）、distance_pct 線性（15%→5/30%+→20）、vol_dryup 線性（0.7→0/0.3→15）、cumul_net intensity 線性（0.05→10/0.20+→15）、large_holder_chg 線性、ma_alignment partial credit（5/pair）✅ · **ChipTransferDetector**：K-of-N gate 保留（≥3 of 5）但每信號 partial credit；margin_streak/price_stability/inst_consec/large_holder/retail_decline/vol_contract 全線性，OHLCV-only mode 同樣連續 ✅ · **VCPDetector**：contraction count 保留（2→15/3+→25 gate-style）、BB compression rank_pct 連續、trough_vol_ratio 線性、pullback_pct 線性、ma_alignment partial credit ✅ · **HTFDetector**：prior_advance / consol_range / vol_contract / position_in_flag / advance_speed 全線性，MA alignment partial credit ✅ · **PullbackDetector**：MA20 proximity 連續 tent（0%→30/±1.5%→25/±5%→10）、vol_contract 線性、ma_slope 線性、bounce candle close_strength 連續、VOL_BOUNCE ratio 連續、RSI reset tent（50 peak 15）、prior_advance 線性、pullback_duration tent（4-6 days peak 10）✅ · K-of-N gates 保留，binary 訊號（MA5>MA60）保留 ✅ · 全引擎完成連續評分，分數撞群問題消除 ✅ · 671 unit tests passing ✅ |
| Phase 4.47 | ✅ Done | **FinMind 付費三大法人 API 取代免費 TWSE T86**：`PaidDataFetcher.fetch_institution_day(date)` 查 `TaiwanStockInstitutionalInvestorsBuySell`（一次 API 拿全市場，含 TSE+TPEx，`{stock_id: (foreign_net, trust_net, dealer_net)}`）✅ · `ChipProxyFetcher._fetch_t86_data` 步驟 0a：`_paid` 存在且該日未快取 → FinMind 填滿 `_t86_date_cache[date]`，完全跳過 TWSE T86 HTTP ✅ · 電路斷路器（circuit breaker）在有付費 API 時不再觸發 ✅ · 免費 T86 僅在 paid fetcher 不可用（無 API Key）或 FinMind 回傳空資料時才作為 fallback ✅ · 8 新單元測試（5 PaidDataFetcher + 3 integration）✅ · 679 unit tests passing ✅ |
| Phase 4.48 | ✅ Done | **資金配置 Tier System + 產業/概念資金流 + 4 新付費因子 + 流動性 bypass 修復**：①**CapitalAllocator** (`domain/capital_allocator.py`) + **AllocationAdvisor** (`agents/allocation_advisor.py`)：讀 rotation_signal/heat/concept_heat JSON，為每訊號算 `rotation_score = 產業熱度×0.5 + 概念熱度×0.3 + 領頭羊×0.2`、偵測產業/題材集中度警告；LLM (cap 35) 給 S/A/B/C tier + 建議配置 % + reasoning；LLM 失敗或 leftover 全歸 C 級 0% ✅ · ② **SectorFlowAnalyzer + ConceptFlowAnalyzer** (`domain/sector_flow.py`)：讀近 N 日 heat_*.json / concept_heat_*.json 時間序列，計算 `rank_delta_total / acceleration_3v3 / trend_direction`（RISING_FAST→DECLINING_FAST 5 階）+ 內嵌 SVG sparkline ✅ · ③ **concepts.json 15→38 baskets**（用戶指定清單 LLM 自動補 23 個 + 過去 10 天 concept_heat backfill 重算）✅ · ④ **4 新付費 FinMind 因子**：`PaidDataFetcher.fetch_futures_context` (TX 期貨外資/投信淨多)、`fetch_options_context` (TXO PCR signal)、`fetch_per_context` (個股 PER/PBR/殖利率)、`fetch_revenue_context` (月營收 YoY) → TCE 新增 `market_regime_bonus_pts (+3 PCR<1+投信多)` / `valuation_pts (PER<15+3, >50-2, 殖利率≥5% +3)` / `revenue_momentum_pts (YoY ≥50% +5, ≥20% +3, 連續 3 月+2)`；TAIFEX gate 整合期貨 + 選擇權空頭訊號降階 LONG→WATCH ✅ · ⑤ **HTML 改版**：header → 產業資金流 → 概念資金流 → Tier link card → filter bar（新增 BB 壓縮未突破 pill + 題材 pill 自動換行）→ cards；資金配置獨立 `allocation_YYYY-MM-DD.html` 新分頁開啟 ✅ · ⑥ **關鍵 bug fix 1 — 流動性 bypass (1817 凱撒衛事件)**：`_scan_pullback_batch` / `_scan_early_accum_batch` 未檢查流動性，把 TCE G3 駁回的低流動性 ticker (1817 20D avg 3.1M < 8M) promote 成 LONG，`_merge_unified_signals` 又把 TCE conf=0 覆蓋成偵測器 conf=98+，Tier 顯示 A 級 conf=111.73 但 DB 無記錄；新增 `_passes_liquidity_floor()` 對齊 TCE G3 (TSE/TPEx 各 NT$8M) ✅ · ⑦ **bug fix 2 — Tier 投入率 1009%**：LLM cap 35 外的 leftover 走 fallback 規則式給 tier，加總爆表；改成 leftover 一律歸 C 級 0% (觀察池) ✅ · ⑧ **bug fix 3 — OpenAIProvider .strip()**：`from_env()` 拿到實例而非字串會炸；改先檢查 `hasattr(llm_provider, "complete")` ✅ · 776 unit tests passing (+22 capital_allocator +26 sector_flow +42 new_factors +7 liquidity_gate) ✅ |
...
| Pillar 2B 三大法人（外資+投信+自營）| ✅（免費 T86 fallback）| ✅（付費 FinMind 優先）| 付費帳號時 FinMind `TaiwanStockInstitutionalInvestorsBuySell`；無付費帳號時 TWSE T86 |
