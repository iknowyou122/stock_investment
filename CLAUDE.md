...
| Phase 4.23 | ✅ Done | **蓄積信號追蹤系統**：`coil_monitor.py` SQLite 追蹤（WAL + busy_timeout）✅ · `db/coil_track.db` 自動建立 ✅ · resistance 在 ingest 時算死（entry_close / (1 + vs_60d_high_pct/100)）✅ · MAE 用 bar low ✅ · win-rate N-guard（N<20 顯示「不足」）✅ · auto-apply 安全閘（90天+每等級50筆）✅ · `coil_scan.py --track` 自動 ingest ✅ · `batch_plan.py` Pass 2 自動 ingest ✅ · `coil_factor_report.py --live-db` ✅ · `optimize_coil.py --live-db --auto-apply` ✅ · bot.py coil panel 追蹤摘要 ✅ · `make flow` 整合 coil-monitor ✅ · 20 unit tests passing ✅ |
