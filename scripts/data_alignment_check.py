...
def run_alignment_check(ticker: str, check_date: date) -> bool:
    ...
    ohlcv_only = ohlcv_dates - broker_dates

    print(f"\nOverlapping dates: {sorted(common)}")
    if broker_only:
        print(f"⚠ Broker-only dates (no OHLCV match): {sorted(broker_only)}")
    if ohlcv_only:
        print(f"⚠ OHLCV-only dates (no broker match): {sorted(ohlcv_only)}")

    # Check that check_date is present in both
...
def main() -> None:
    ...
    passed = run_alignment_check(args.ticker, check_date)
